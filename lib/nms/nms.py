...
def py_hybrid_wrapper(thresh, max_dets=-1):
    def _nms(dets):
        return hybrid_nms(dets, thresh, max_dets)
    return _nms
...
def hybrid_nms(dets, thresh, max_dets):
    if dets.shape[0] == 0:
        return np.zeros((0, 5))
    ...
    while order.size > 0 and keep_cnt < max_dets:
        ...
        inds = np.where(ovr <= thresh)[0]
        order = order[inds + 1]
        scores = rescore(ovr[inds], scores[inds + 1], thresh)
