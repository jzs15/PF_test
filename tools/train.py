...
from symbols import *
from core.loader import AnchorLoader, PyramidAnchorIterator
...
def train_fpn(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    mx.random.seed(3)
    np.random.seed(3)
...
def train_faster(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    logger, final_output_path = create_logger(config.output_path, args.cfg, config.dataset.image_set)
    prefix = os.path.join(final_output_path, prefix)
...
def main():
    ...
    if config.network.FPN:
        train_net = train_fpn
    else:
        train_net = train_faster
